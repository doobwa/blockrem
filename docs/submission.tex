...
where $n_k=\sum_{i}I(z_i^{split}=k)$.% and compute $q( \mathbf{z}^{merge} \rightarrow \mathbf{z}^{split})$ to be the product of the probabilities for the sampled assignments.
...
\begin{itemize}
\item For all blocks $r \ne k,l$: sample $\phi_{kr}^{merge} \sim \mbox{Unif}(\{\phi_{kr}^{split}, \phi_{lr}^{split}\})$, and $\phi_{rk}^{merge} \sim \mbox{Unif}(\{\phi_{rk}^{split}, \phi_{rl}^{split}\})$.%, and sample $\phi_{lr}^{merge},\phi_{rl}^{merge}$, and $\phi_{ll}^{merge}$ independently from the prior.
