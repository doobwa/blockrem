...
%In this case, we have a count for the total activity of each dyad, and one may analyze it has a static network. 
%Methods for static networks such as a 
...
Network-based event data are powerful, however, because we enable the study of the processes governing these events.
Theories about the dynamics among nodes will be often be difficult to study when only using the marginal rate of events as illustrated in Figure \ref{fig:introexample}
For example, a common aspect of human communication is \emph{reciprocity} --- after person A speaks to person B, B subsequently has a higher propensity to speak to A.  
These sorts of dynamics may be studied with event data, but again, our focus may be on the differences in these dynamics among the nodes in the network.  
In the context of studying interactions among students, one might be interested particular conversational tendencies are associated with covariates about the student or the classroom.  
A standard blockmodel only provides a means for studying the structure of marginal rates of communication, but not the shared tendencies in the dynamics of the sequence of events.
