...
In such settings we often observe events occurring between nodes over time, and a common goal is to understand the differences in the total number of interactions --- perhaps with respect to known information about the nodes.
...
Through an event-based analysis we can study the similarity --- and differences --- in the dynamics of nodes in the network. 
When studying interactions among students, for example, one might be interested in conversational tendencies are associated with covariates about the student or classroom, or based on unobserved groupings.  
In this context, a standard blockmodel only provides a means for studying the structure of marginal rates of communication, but not the shared tendencies in the dynamics of the sequence of events.
