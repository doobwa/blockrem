...
Rcpp::List llk(Rcpp::NumericVector beta, Rcpp::NumericVector times, Rcpp::IntegerVector sen, Rcpp::IntegerVector rec, Rcpp::IntegerVector ix, Rcpp::IntegerVector jx, int N, int M, int P){
  ...
  for (int m = 1; m<M; m++) {
    ...
    for (int v = 0; v < jx.size(); v++) {
      ...
      a = sen[mp(i,r)];
      b = rec[mp(i,r)];
      lam = computeLambda(i,r,a,b,beta);
      llk -= (times[m] - times[mp(i,r)]) * exp(lam);
      mp(i,r) = m;  // update mp
      a = sen[mp(r,i)];
      b = rec[mp(r,i)];
      lam = computeLambda(r,i,a,b,beta);
    ...
    for (int v = 0; v < ix.size(); v++) {
      ...
      a = sen[mp(j,r)];
      b = rec[mp(j,r)];
      lam = computeLambda(j,r,a,b,beta);
      llk -= (times[m] - times[mp(j,r)]) * exp(lam);
      mp(j,r) = m;  // update mp
      a = sen[mp(r,j)];
      b = rec[mp(r,j)];
      lam = computeLambda(r,j,a,b,beta);
