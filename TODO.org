...
- Fix bug where K=5 z vector has none of one of the clusters (or something)
- Try eckmann-small and synthetic dataset with degree effects.  Tinker
  with simulating with degree effects.  Perhaps come up with other
  parameterization that is less sensitive to explosion.  
  - fit via ML with fixed cluster assignments.
  - Ans: ML performs quite poorly on synthetic data.
...
- consider using features relating to how many times a particular dyad
  occurred recently?  Or how many changepoints exist in a particular
  window?
  - quick version: just include # times this dyad has occurred
    preivously
  - try fitting the model with degree effects and K=1 via ML or something.
...
- Write abstract.
- Add Enron.
- Add UC Irvine dataset?  (Make subset of it.)
